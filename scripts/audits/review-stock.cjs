...
const professions = JSON.parse(fs.readFileSync(path.join(dataDir, "market", "professions.json"), "utf8"));
...
const stockDir = path.join(root, "src", "content", "market", "stock");
...
for (const slug of reviewSlugs) {
  ...
  lines.push(`- Runtime stock bias preview: ${topBiases || "none"}.`);
  if (balance) {
...
lines.push("- Runtime stock bias uses local `stockBiasWeights` on each resolved profile: character and profession biases are strongest, market and kingdom biases are lighter, and illegal kingdom tags discourage legal merchants while helping criminal stock.");

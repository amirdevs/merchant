...
function readableText(itemName: string, tags: string[], marketName: string) {
  if (tags.some((tag) => ["books", "book", "letters", "letter"].includes(tag.toLowerCase()))) {
      ...
      <Panel className="relative w-full max-w-4xl" title="Item Detail" variant="parchment">
        ...
        <div className="grid gap-5 md:grid-cols-[260px_1fr]">
          ...
          <div>
            ...
            <div className="mt-4 rounded-md border border-[#9a7138]/60 bg-[#fff6d7]/45 p-3 text-sm text-[#3b260f]"><p><strong>Market demand:</strong> {(market.bias || []).filter((bias) => item.tags.includes(bias.tag) || item.name === bias.tag).map((bias) => `${bias.tag} ${bias.percent > 0 ? "+" : ""}${bias.percent}%`).join(", ") || "No specific local modifier."}</p><p><strong>Legality:</strong> {illegal ? `illegal in ${kingdom?.name || market.name}` : `legal in ${kingdom?.name || market.name}`}.</p></div>
            {readerOpen ? <div className="mt-4 rounded-md border-2 border-[#1f5960]/55 bg-[#fff8df]/85 p-4 text-[#3b260f]"><strong className="font-display text-2xl">Read / Inspect</strong><p className="mt-2 leading-relaxed">{readableText(item.name, item.tags, market.name)}</p></div> : null}

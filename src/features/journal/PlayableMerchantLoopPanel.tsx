...
import { Building2, CheckCircle2, Coins, Map, PackagePlus, Play, RotateCcw, Route, Store } from "lucide-react";
  ...
  return (
    <Panel title={<span className="inline-flex items-center gap-2"><Route size={18} /> Playable Merchant Loop v1</span>} variant="parchment">
      ...
      <p className="mb-4 rounded-sm border border-[#9a7138]/50 bg-[#fff6d7]/55 p-3 text-sm font-bold leading-snug text-[#3b260f]">
        This is the first full playable merchant loop: buy cargo, travel to another town, sell for profit, progress the rich story chain, and register the first company.
      </p>
        ...
        <div className="grid gap-4">
          ...
          <div className="grid gap-3 lg:grid-cols-2">
            <div className="rounded-sm border border-[#9a7138]/50 bg-[#fff6d7]/60 p-3 text-sm text-[#3b260f]">
              <strong className="block text-[#26170a]">Local Prices</strong>
            ...
            <div className="rounded-sm border border-[#9a7138]/50 bg-[#fff6d7]/60 p-3 text-sm text-[#3b260f]">
              <strong className="block text-[#26170a]">Best Nearby Profit Routes</strong>
              <div className="mt-2 grid gap-2">
                {view.tradeRoutes.map((route) => (
                  <div className="rounded-sm border border-[#9a7138]/40 bg-[#fff6d7]/65 p-2" key={`${route.routeId}-${route.itemId}`}>
                    ...
                    <span className="block font-bold text-[#1f5960]">Buy {route.buyPrice}c / Sell {route.sellPrice}c / Net {route.netProfit}c</span>
                  </div>
                ))}
              </div>
            </div>
